...
from ..services.fiscal import get_current_year, get_prior_year_end_date, is_cy
...
def income_statement(db: Session = Depends(get_db)) -> IncomeStatementOut:
    ...
    def section_total(groups: list[IncomeStatementGroupOut], favorable_when_actual_higher: bool) -> IncomeStatementRowOut:
        return _row(
            "Total",
    ...
    return IncomeStatementOut(
        ...
        income_total=section_total(income_groups, favorable_when_actual_higher=True),
        expense_groups=expense_groups,
        expense_total=section_total(expense_groups, favorable_when_actual_higher=False),
    )
